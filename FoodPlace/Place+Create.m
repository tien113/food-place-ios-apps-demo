...
+ (Place *)placeWithWebService:(NSDictionary *)webService inManagedObjectContext:(NSManagedObjectContext *)context {
    ...
    NSError *error = nil;
    NSArray *places = [context executeFetchRequest:request
                                             error:&error]; // fetch all places from Core Data
    ...
    if (!places || places.count > 1) {
        ...
        place.lat            = [NSDecimalNumber decimalNumberWithString:webService[PLACE_LAT]]; // convert string to decimal number
        place.log            = [NSDecimalNumber decimalNumberWithString:webService[PLACE_LOG]]; // convert string to decimal number
